...
function getDefaultFilters(data: ComparisonData | null): FilterState {
  ...
  // Default to East Africa region (Global is excluded from this market)
  const firstGeography =
    data.dimensions.geographies.regions?.[0] ||
    data.dimensions.geographies.all_geographies?.find((g) => g !== 'Global') ||
    ''
